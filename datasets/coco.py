...
from commons.augmentations import Compose, OneOf, \
    ScaleMinMax, RandNoise, RandPerspective, HSV, Identity, LRFlip, BatchPadding, Mosaic
...
class COCODataSets(Dataset):
    def __init__(self, img_root, annotation_path,
        ...
        self.transform = None
        self.set_transform()
        self.batch_transform = BatchPadding(padding_value=self.aug_cfg['pad_val'], center_padding=True)
    ...
    def set_transform(self):
        if self.augments:
            ...
            basic_transform = Compose(transforms=[
                color_jitter,
                ScaleMinMax(min_thresh=self.min_thresh, max_thresh=self.max_thresh),
            ])
            mosaic_transform = Mosaic(
                self.img_paths,
                self.labels,
