...
class ScaleMinMax(DetectAugment):
    # 调整图像大小至[600,1024]之间,保证短边等于600,长边小于1024
    def __init__(self, min_thresh=600, max_thresh=1024, p=1):
        super(ScaleMinMax, self).__init__(p=p)
...
class Mosaic(DetectAugment):
    ...
    def aug(self, img: np.ndarray, labels: np.ndarray) -> tuple:
        ...
        if len(labels4):
            ...
            img4, labels = self.affine(img, labels)
            return img4, labels
...
class BatchPadding(object):
    ...
    def __call__(self, imgs, labels):
        """
        :param imgs(list): img ("bgr" channel order)
        :param labels(list): [gt_num,6] (weight,label_idx,x1,y1,x2,y2) (img scale)
        :return: ret_imgs [batch_size,h,w,3];ret_labels [gt_nums,7]
        """
        ...
        for i, (img, label) in enumerate(zip(imgs, labels)):
            ...
            if len(label):
                # [batch_idx,weights,label_idx,x1,y1,x2,y2]
                temp_label = np.empty(shape=(label.shape[0], label.shape[1] + 1))
                temp_label[..., 0] = i
                temp_label[..., [1, 2]] = label[..., [0, 1]]
                temp_label[..., [3, 5]] = (label[..., [2, 4]] + left) / max_w
                temp_label[..., [4, 6]] = (label[..., [3, 5]] + top) / max_h
                # import uuid
                # from commons.boxs_utils import draw_box
                # from datasets.coco import colors, coco_names
                # temp_label[..., [3, 5]] = temp_label[..., [3, 5]] * max_w
                # temp_label[..., [4, 6]] = temp_label[..., [4, 6]] * max_h
            ...
            if self.rgb_mean is not None and self.rgb_std is not None:
                ret_img = ((ret_img[..., ::-1].astype(np.float32) / 255.0) - np.array(self.rgb_mean)) \
                          / np.array(self.rgb_std)
